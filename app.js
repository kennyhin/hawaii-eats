...
function skinBackgroundCss(skin) {
  ...
  if (skin.image) {
    return `${wash}, url('${skin.image}?v=20260623f') center/cover no-repeat`;
  }
...
function renderGameCanvas(name) {
  ...
  if (courtItem?.image) {
    ...
    courtImageEl.src = `${courtItem.image}?v=20260623f`;
  }
...
// ---------- Runner mini-game ----------
// A Temple-Run-style endless lane runner, open to everyone (no allowlist).
// Entry costs the same flat fee as Tennis, deducted up front; whatever is
// earned during the run (map clears + coins) is credited on top. Crashing
// loses the whole run — the next attempt starts back at Map 1 and pays a
// fresh entry fee, same way a new Tennis series or Blackjack hand would.
const RUNNER_DIFFICULTY = {
  easy: { label: 'Easy', emoji: '🟢', speedMult: 1, mapPassPoints: 5 },
  hard: { label: 'Hard', emoji: '🔴', speedMult: 1.55, mapPassPoints: 7 },
};
// Every map lasts the same fixed 30 seconds — difficulty progression comes
// from speed/density (below) and from the easy/hard speedMult above, not
// from the map getting longer.
const RUNNER_MAP_DURATION_MS = 30000;
const RUNNER_MAPS = [
  { baseSpeed: 5.0, spawnGap: 340, doubleLaneChance: 0.05 },
  { baseSpeed: 5.6, spawnGap: 310, doubleLaneChance: 0.10 },
  { baseSpeed: 6.2, spawnGap: 280, doubleLaneChance: 0.16 },
  { baseSpeed: 6.8, spawnGap: 250, doubleLaneChance: 0.22 },
  { baseSpeed: 7.4, spawnGap: 220, doubleLaneChance: 0.30 },
];
const RUNNER_COIN_VALUE = 5;
const RUNNER_COIN_CHANCE = 0.32;
const RUNNER_FAR_Z = 1600;
const RUNNER_JUMP_DURATION = 380;
const RUNNER_DIFFICULTY_KEY = 'food_memory_album_runner_difficulty';

function getSavedRunnerDifficulty() {
  return localStorage.getItem(RUNNER_DIFFICULTY_KEY) || 'easy';
}
function saveRunnerDifficulty(difficulty) {
  localStorage.setItem(RUNNER_DIFFICULTY_KEY, difficulty);
}
...
function openRunner() {
  ...
  const profile = name ? getProfile(name) : null;
  const available = name ? (getRawPoints(name) - (profile?.spentPoints || 0)) : 0;
  const difficulty = getSavedRunnerDifficulty();
  const modal = document.getElementById('runnerModal');
  modal.innerHTML = `
    <button class="modal-close" id="runnerCloseBtn">✕</button>
    <h2>🏃 Runner</h2>
    <p class="hint">Race through 5 maps, each a 30-second dash that gets a little harder each time. Entry costs ${GAME_ENTRY_COST} pts no matter the difficulty. Grab coins along the way for +${RUNNER_COIN_VALUE} pts each — crash and the run (and entry fee) is over. Drag left/right to switch lanes, tap to jump.</p>
    <div class="shop-name-field">
      <label>Who's running?</label>
      <input type="text" id="runnerNameInput" placeholder="Your name" value="${escapeHtml(name)}">
    </div>
    <div class="shop-name-field">
      <label>Choose your difficulty</label>
      <div class="tennis-difficulty-picker">
        ${Object.entries(RUNNER_DIFFICULTY).map(([id, cfg]) => `
          <button type="button" class="tennis-diff-btn ${difficulty === id ? 'active' : ''}" data-diff="${id}">
            <span class="tennis-diff-label">${cfg.emoji} ${cfg.label}</span>
            <span class="tennis-diff-prize">Clear +${cfg.mapPassPoints}</span>
          </button>
        `).join('')}
      </div>
    </div>
    ${name ? `<p class="shop-balance">You have <strong>${available}</strong> pt${available === 1 ? '' : 's'} to spend</p>` : `<p class="hint">Type your name above to play.</p>`}
    <div class="modal-actions">
      <button type="button" class="btn btn-primary" id="runnerStartBtn" ${(!name || available < GAME_ENTRY_COST) ? 'disabled' : ''}>🏃 Start Run (-${GAME_ENTRY_COST} pts)</button>
    </div>
  `;
  document.getElementById('runnerCloseBtn').addEventListener('click', closeRunner);
  document.getElementById('runnerNameInput').addEventListener('change', (e) => {
    saveAuthorName(e.target.value.trim());
    openRunner();
  });
  document.querySelectorAll('#runnerModal .tennis-diff-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      saveRunnerDifficulty(btn.dataset.diff);
      openRunner();
    });
  });
  const startBtn = document.getElementById('runnerStartBtn');
  if (startBtn) {
    startBtn.addEventListener('click', () => startRunnerRun(document.getElementById('runnerNameInput').value.trim()));
  }
  document.getElementById('runnerOverlay').classList.remove('hidden');
}

function closeRunner() {
  if (runnerGame && runnerGame.running) {
    if (!confirm('Leave the run? Your progress (and entry fee) will be lost.')) return;
  }
  stopRunnerLoop();
...
function stopRunnerLoop() {
  if (runnerGame) runnerGame.running = false;
  if (runnerAnimId) cancelAnimationFrame(runnerAnimId);
  runnerAnimId = null;
  runnerGame = null;
}

async function startRunnerRun(name) {
  if (!name) return;
  const key = normalizeName(name);
  const profile = getProfile(name) || { spentPoints: 0, unlocked: [], credits: [] };
  const available = getRawPoints(name) - (profile.spentPoints || 0);
  ...
  try {
    ...
    console.error(err);
    alert('Could not start the run — check your internet connection and try again.');
    return;
  }
  renderRunnerCanvas(name, 0, 0);
}

function renderRunnerCanvas(name, mapIndex, totalEarned) {
  const difficulty = getSavedRunnerDifficulty();
  const diffConfig = RUNNER_DIFFICULTY[difficulty];
  const mapConfig = RUNNER_MAPS[mapIndex];

  const modal = document.getElementById('runnerModal');
  modal.innerHTML = `
    <button class="modal-close" id="runnerCloseBtn">✕</button>
    <h2 id="runnerTitle">🏃 Map ${mapIndex + 1} / ${RUNNER_MAPS.length} · ${diffConfig.emoji} ${diffConfig.label}</h2>
    <div class="runner-hud">
      <div class="runner-progress-track"><div class="runner-progress-fill" id="runnerProgressFill"></div></div>
      <span id="runnerEarnedLabel">Earned: ${totalEarned} pts</span>
    </div>
    <div class="runner-canvas-wrap">
      <canvas id="runnerCanvas" width="600" height="360"></canvas>
      <div class="tennis-start-overlay" id="runnerStartOverlay">
        <button type="button" class="btn btn-primary" id="runnerStartMapBtn">▶ Start Map ${mapIndex + 1}</button>
      </div>
    </div>
    <p class="hint">Drag your finger left/right to switch lanes — tap to jump over obstacles in your lane.</p>
  `;
  document.getElementById('runnerCloseBtn').addEventListener('click', closeRunner);
  ...
  runnerGame = {
    name, canvas, ctx,
    w: canvas.width, h: canvas.height,
    mapIndex, mapConfig, difficulty,
    speed: mapConfig.baseSpeed * diffConfig.speedMult,
    elapsedMs: 0,
  ...
  const overlay = document.getElementById('runnerStartOverlay');
  document.getElementById('runnerStartMapBtn').addEventListener('click', () => {
    overlay.classList.add('hidden');
    runnerGame.paused = false;
    runnerGame.lastFrameTime = null;
    runnerAnimId = requestAnimationFrame(runnerLoop);
  });

  let gestureStartX = null, gestureStartY = null, gestureStartTime = 0, gestureMoved = false;
  ...
  canvas.addEventListener('pointermove', (e) => {
    if (gestureStartX === null || !runnerGame) return;
    const dx = e.clientX - gestureStartX;
    if (Math.abs(dx) > 40) {
      runnerGame.lane = Math.max(0, Math.min(2, runnerGame.lane + (dx > 0 ? 1 : -1)));
  ...
  canvas.addEventListener('pointerup', (e) => {
    ...
    const totalMove = Math.hypot(e.clientX - gestureStartX, e.clientY - gestureStartY);
    if (!gestureMoved && elapsed < 350 && totalMove < 20 && !runnerGame.paused) {
      runnerJump();
    }
    gestureStartX = null;
  });

  drawRunner(runnerGame);
}

function runnerJump() {
  if (!runnerGame || runnerGame.jumping) return;
  runnerGame.jumping = true;
  runnerGame.jumpStart = Date.now();
}

function runnerRoadX(offsetUnits, t) {
  const halfWidth = lerp(260, 16, t);
  return 300 + offsetUnits * (halfWidth / 1.5);
}
function runnerRoadY(t) {
  return lerp(332, 92, t);
}
function runnerScale(t) {
  return lerp(1, 0.18, t);
}
function lerp(a, b, t) {
  return a + (b - a) * t;
}
function runnerEaseT(z) {
  const t = Math.max(0, Math.min(1, z / RUNNER_FAR_Z));
  return Math.pow(t, 1.25);
}
function runnerLaneOffset(lane) {
  return (lane - 1) * 1.0;
}

function runnerSpawn(g) {
  const isCoin = Math.random() < RUNNER_COIN_CHANCE;
  if (isCoin) {
    g.coins.push({ z: RUNNER_FAR_Z, lane: Math.floor(Math.random() * 3), collected: false });
    return;
  }
  const isDouble = Math.random() < g.mapConfig.doubleLaneChance;
  if (isDouble) {
    const safeLane = Math.floor(Math.random() * 3);
    const lanes = [0, 1, 2].filter(l => l !== safeLane);
    g.obstacles.push({ z: RUNNER_FAR_Z, lanes });
  } else {
    g.obstacles.push({ z: RUNNER_FAR_Z, lanes: [Math.floor(Math.random() * 3)] });
  }
}

function runnerLoop() {
  if (!runnerGame || !runnerGame.running || runnerGame.paused) return;
  const g = runnerGame;

  // requestAnimationFrame isn't reliably capped to 60Hz (high refresh-rate
  // phones, or an uncapped headless browser, fire it much more often), so
  // movement is scaled by real elapsed time instead of a fixed per-frame
  // step — otherwise the run plays faster on some devices than others.
  const now = performance.now();
  if (g.lastFrameTime == null) g.lastFrameTime = now;
  const realDeltaMs = now - g.lastFrameTime;
  const dt = Math.max(0, Math.min(2, realDeltaMs / (1000 / 60)));
  g.lastFrameTime = now;
  const step = g.speed * dt;

  // Clamp how much real time can count toward the 30s clock in one frame —
  // otherwise resuming a backgrounded tab could insta-finish the map.
  g.elapsedMs += Math.max(0, Math.min(250, realDeltaMs));

  g.laneVisual += (g.lane - g.laneVisual) * Math.min(1, 0.28 * dt);

  if (g.jumping && Date.now() - g.jumpStart > RUNNER_JUMP_DURATION) {
    g.jumping = false;
  }

  g.distanceSinceSpawn += step;
  if (g.distanceSinceSpawn >= g.mapConfig.spawnGap) {
    g.distanceSinceSpawn = 0;
    runnerSpawn(g);
  }

  g.obstacles.forEach(o => { o.z -= step; });
  g.coins.forEach(c => { c.z -= step; });

  // Collision/pickup window — entities crossing near the player this frame.
  // Wide enough that even a clamped dt=2 step at top speed can't skip through.
  for (const o of g.obstacles) {
    if (!o.hit && !o.passed && o.z <= 24 && o.z >= -24) {
      if (o.lanes.includes(g.lane) && !g.jumping) {
        o.hit = true;
        runnerCrash();
        return;
      }
      if (o.z <= -24) o.passed = true;
    }
  }
  for (const c of g.coins) {
    if (!c.collected && c.z <= 24 && c.z >= -24 && c.lane === g.lane) {
      c.collected = true;
      g.coinsThisMap += 1;
    }
  }
  g.obstacles = g.obstacles.filter(o => o.z > -60);
  g.coins = g.coins.filter(c => c.z > -60 && !c.collected || (c.collected && c.z > -10));

  drawRunner(g);

  const fillPct = Math.min(100, (g.elapsedMs / RUNNER_MAP_DURATION_MS) * 100);
  const fillEl = document.getElementById('runnerProgressFill');
  if (fillEl) fillEl.style.width = fillPct + '%';

  if (g.elapsedMs >= RUNNER_MAP_DURATION_MS) {
    runnerMapPassed();
    return;
  }
...
function drawRunner(g) {
  ...
  // Sky/horizon glow
  const grad = ctx.createLinearGradient(0, 0, 0, runnerRoadY(1));
  grad.addColorStop(0, '#1d3a52');
  grad.addColorStop(1, '#0b1f2e');
  ctx.fillStyle = grad;
  ctx.fillRect(0, 0, g.w, runnerRoadY(1));

  // Road polygon
  ctx.fillStyle = '#2b2f36';
  ctx.beginPath();
  ctx.moveTo(runnerRoadX(-1.5, 0), runnerRoadY(0));
  ctx.lineTo(runnerRoadX(1.5, 0), runnerRoadY(0));
  ctx.lineTo(runnerRoadX(1.5, 1), runnerRoadY(1));
  ctx.lineTo(runnerRoadX(-1.5, 1), runnerRoadY(1));
  ctx.closePath();
  ctx.fill();
  ...
  [-0.5, 0.5].forEach(offset => {
    ctx.beginPath();
    for (let z = 0; z <= RUNNER_FAR_Z; z += 80) {
      const t = runnerEaseT(z);
      const x = runnerRoadX(offset, t);
      const y = runnerRoadY(t);
  ...
  g.obstacles.forEach(o => {
    if (o.z < -60 || o.z > RUNNER_FAR_Z) return;
    const t = runnerEaseT(Math.max(0, o.z));
    const y = runnerRoadY(t);
    const scale = runnerScale(t);
    const minLane = Math.min(...o.lanes), maxLane = Math.max(...o.lanes);
    const leftX = runnerRoadX(runnerLaneOffset(minLane) - 0.4, t);
    const rightX = runnerRoadX(runnerLaneOffset(maxLane) + 0.4, t);
    ctx.fillStyle = '#d62839';
    ctx.fillRect(leftX, y - 26 * scale, rightX - leftX, 26 * scale);
  });

  // Coins
  g.coins.forEach(c => {
    if (c.collected || c.z < -60 || c.z > RUNNER_FAR_Z) return;
    const t = runnerEaseT(Math.max(0, c.z));
    const y = runnerRoadY(t);
    const x = runnerRoadX(runnerLaneOffset(c.lane), t);
    const r = 11 * runnerScale(t);
    ctx.fillStyle = '#ffd23f';
    ctx.beginPath();
    ctx.arc(x, y - 14 * runnerScale(t), r, 0, Math.PI * 2);
    ctx.fill();
  ...
  // Player
  const px = runnerRoadX(runnerLaneOffset(g.laneVisual), 0);
  const py = runnerRoadY(0);
  let jumpOffset = 0;
  if (g.jumping) {
    const elapsed = Date.now() - g.jumpStart;
    jumpOffset = 60 * Math.sin(Math.PI * Math.min(1, elapsed / RUNNER_JUMP_DURATION));
  }
  const legSwing = g.paused ? 0 : Math.sin(Date.now() / 90) * 8;
  ctx.save();
  ...
  ctx.lineWidth = 6;
  ctx.beginPath();
  ctx.moveTo(-4, -14); ctx.lineTo(-4 + legSwing, 6);
  ctx.moveTo(4, -14); ctx.lineTo(4 - legSwing, 6);
  ctx.stroke();
  ctx.restore();
}

async function runnerMapPassed() {
  const g = runnerGame;
  g.running = false;
  cancelAnimationFrame(runnerAnimId);

  const mapPassPoints = RUNNER_DIFFICULTY[g.difficulty].mapPassPoints;
  const mapPoints = mapPassPoints + g.coinsThisMap * RUNNER_COIN_VALUE;
  const name = g.name;
  const key = normalizeName(name);
  const profile = getProfile(name) || { spentPoints: 0, unlocked: [], credits: [] };
  const reason = g.coinsThisMap > 0
    ? `cleared Map ${g.mapIndex + 1} of the Runner and grabbed ${g.coinsThisMap} coin${g.coinsThisMap === 1 ? '' : 's'}`
    : `cleared Map ${g.mapIndex + 1} of the Runner`;
  const credits = [...(profile.credits || []), { points: mapPoints, displayPoints: mapPoints, reason, source: 'game', gameType: 'runner', awardedAt: Date.now() }];
  const updates = { displayName: name, credits };
  try {
    await setDoc(doc(db, PROFILES_COLLECTION, key), updates, { merge: true });
    profiles[key] = { ...profile, ...updates };
  ...
  const totalEarned = g.totalEarned + mapPoints;
  const isLastMap = g.mapIndex >= RUNNER_MAPS.length - 1;

  const modal = document.getElementById('runnerModal');
  modal.innerHTML = `
    <button class="modal-close" id="runnerCloseBtn">✕</button>
    <div class="randomizer-result">
      <div class="randomizer-dice">${isLastMap ? '🎉' : '✅'}</div>
      <h2>${isLastMap ? 'You cleared all 5 maps!' : `Map ${g.mapIndex + 1} complete!`}</h2>
      <p class="hint">+${mapPoints} pts this map (${totalEarned} pts earned this run so far).</p>
    </div>
    <div class="modal-actions">
      ${isLastMap
        ? `<button type="button" class="btn btn-primary" id="runnerPlayAgainBtn">🏃 Play Again</button>`
        : `<button type="button" class="btn btn-primary" id="runnerNextMapBtn">▶ Continue to Map ${g.mapIndex + 2}</button>`}
      <button type="button" class="btn btn-secondary" id="runnerDoneBtn">Close</button>
    </div>
  `;
  document.getElementById('runnerCloseBtn').addEventListener('click', closeRunner);
  document.getElementById('runnerDoneBtn').addEventListener('click', closeRunner);
  if (isLastMap) {
    document.getElementById('runnerPlayAgainBtn').addEventListener('click', () => openRunner());
  } else {
    document.getElementById('runnerNextMapBtn').addEventListener('click', () => {
      renderRunnerCanvas(name, g.mapIndex + 1, totalEarned);
    });
  }
}
...
function runnerCrash() {
  ...
  modal.innerHTML = `
    ...
    <div class="randomizer-result">
      ...
      <h2>You crashed on Map ${g.mapIndex + 1}!</h2>
      <p class="hint">${g.totalEarned > 0 ? `You still keep the ${g.totalEarned} pts already earned this run.` : `Your ${GAME_ENTRY_COST} pt entry fee is gone — better luck next time!`}</p>
    </div>
    <div class="modal-actions">
      <button type="button" class="btn btn-primary" id="runnerPlayAgainBtn">🏃 Play Again</button>
      <button type="button" class="btn btn-secondary" id="runnerDoneBtn">Close</button>
...
function activityIcon(type, gameType) {
  if (type === 'game_result' && gameType === 'runner') return '🏃';
...
function renderActivityFeed() {
  ...
  list.innerHTML = feed.map(item => `
    <div class="activity-item ${item.placeId ? '' : 'activity-item-static'}" ${item.placeId ? `data-place-id="${item.placeId}"` : ''}>
      <span class="activity-icon">${activityIcon(item.type, item.gameType)}</span>
...
document.getElementById('runnerBtn').addEventListener('click', () => { closeAppMenu(); openRunner(); });
